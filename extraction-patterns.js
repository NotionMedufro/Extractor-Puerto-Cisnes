...
const EXTRACTION_PATTERNS = {
    ...
    hemograma: {
        ...
        leucocitos: /(?:RECUENTO DE LEUCOCITOS|LEUCOCITOS)\s+(?:[hi]\s+)?(\d+\.?\d*)\s+(?:10\^3|10e3)\/uL/i,
    ...
    renal: {
        ...
        bun: /(?:NITROGENO UREICO \(BUN\)|BUN:)\s+([hi]?\s*\d+\.?\d*)\s+mg(?:%|\/dl)/i,
    ...
    hepatico: {
        ...
        bilirrubina_directa: /(?:Bilirrubina Directa|BILIRRUBINA DIRECTA:?)[\s\S]*?(\*?\s*\d+\.?\d*)\s+mg\/dL/i,

        // GOT/ASAT - NUEVO FORMATO: "ASPARTATO AMINO TRANSFERASA (ASAT/GOT) 29.20 U/L"
        got: /(?:ASPARTATO AMINO TRANSFERASA.*?\(ASAT\/GOT\)|G\.O\.T:)\s+(?:[hi]\s+)?(\d+\.?\d*)\s+(?:U\/L|UI\/l)/i,

        // GPT/ALT - NUEVO FORMATO: "ALANINA AMINO TRANSFERASA (ALAT/GPT) h 50.10 U/L"
        gpt: /(?:ALANINA AMINO TRANSFERASA.*?\(ALAT\/GPT\)|G\.P\.T:)\s+(?:[hi]\s+)?(\d+\.?\d*)\s+(?:U\/L|UI\/l)/i,

        // Fosfatasa Alcalina - NUEVO FORMATO: "FOSFATASAS ALCALINAS h 174.00 U/L"
        fosfatasa_alcalina: /(?:FOSFATASAS ALCALINAS|FOSFATASA ALCALINA:?)\s+(?:[hi]\s+)?(\d+\.?\d*)\s+(?:U\/L|UI\/l)/i,

        // GGT - No encontrada en nuevo formato, mantener original
        ggt: /(?:Gamma Glutamiltranspeptidasa|GGT)[\s\S]*?(\*?\s*\d+\.?\d*)\s+(?:U\/L|UI\/l)/i,
    ...
    nutricional: {
        ...
        colesterol_total: /COLESTEROL TOTAL:\s+([hi]?\s*\d+\.?\d*)\s+mg\/dl/i,
        ldl: /COLESTEROL LDL:\s+([hi]?\s*\d+\.?\d*)\s+mg\/dl/i,
        hdl: /COLESTEROL HDL:\s+([hi]?\s*\d+\.?\d*)\s+mg\/dl/i
    },
